...
SNP Preprocessing Code

Transposing Embark Data
```{r}
# Load required libraries
library(readr)

# Step 1: Read your CSV data (assuming your genetic data is in a CSV file)
csv_file <- "UntransposedReadyforVCF.csv"
col_types <- cols(SNP = col_character())  # Assuming SNP column contains strings
genetic_data <- read_csv(csv_file, col_types = col_types)

# Step 2: Transpose the data using 't()' function and convert back to a data frame
transposed_genetic_data <- as.data.frame(t(genetic_data))

# Step 3: Save the transposed data to a new CSV file
output_file <- "TransposedFull.csv"
write_csv(transposed_genetic_data, output_file)
```

Standardizing SNP allele codings
```{r}
library(stringr)
library(data.table)

data8 = fread("FilePath/FileName.csv")
data8[] <- lapply(data8, function(x) str_replace_all(x, "AA", "A A"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "AC", "A C"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "AG", "A G"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "AT", "A T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "CA", "A C"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "C A", "A C"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "CC", "C C"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "CG", "C G"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "CT", "C T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "GA", "A G"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "G A", "A G"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "GC", "C G"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "G C", "C G"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "GG", "G G"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "GT", "G T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "TA", "A T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "T A", "A T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "TC", "C T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "T C", "C T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "TG", "G T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "T G", "G T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "TT", "T T"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "II", "I I"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "DI", "D I"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "ID", "D I"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "I D", "D I"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "DD", "D D"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "0", "0 0"))
data8[] <- lapply(data8, function(x) str_replace_all(x, "00", "0 0"))

write.csv(data8, file = "StandardizedCandidateSNPs.csv", row.names = FALSE)
```


SNP Encoding using incomplete dominance model
```{r}
# Load the required libraries (if not already loaded)
library(data.table)

# Replace "/Users/williamrussel/Desktop/EyeAreaFilteredSNPs.csv" with your CSV file's name
file_path <- "/Users/williamrussel/Desktop/LongCoatFilteredSNPs.csv"

# Read the data from the CSV file using data.table for faster reading
data <- fread(file_path)

# Function to create one-hot encoded SNP columns
one_hot_encode_snps <- function(snp_column, snp_name) {
  snp_id_1 <- as.integer(snp_column == "1")
  snp_id_2 <- as.integer(snp_column == "2")
  one_hot_encoded_snp <- data.frame(snp_id_1, snp_id_2)
  colnames(one_hot_encoded_snp) <- c(paste0(snp_name, "_1"), paste0(snp_name, "_2"))
  return(one_hot_encoded_snp)
}

# Apply one-hot encoding to each column and combine them with their names
one_hot_encoded_df_list <- mapply(one_hot_encode_snps, data, names(data), SIMPLIFY = FALSE)

# Combine the one-hot encoded data frames into a single data frame
one_hot_encoded_data <- as.data.frame(do.call(cbind, one_hot_encoded_df_list))

# Print the resulting data
print(one_hot_encoded_data)

# Save the one-hot encoded data to a new CSV file
output_file <- file.path("~/Desktop", "LongCoatFilteredSNPsEncoded.csv")
write.csv(one_hot_encoded_data, file = output_file, row.names = FALSE)
```
